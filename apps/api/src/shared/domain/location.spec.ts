...
describe('Location value object', () => {
  ...
  it('accepts boundary latitude -90', () => {
    expect(() =>
      Location.create({ address: 'South Pole', latitude: -90, longitude: 0 }),
    ).not.toThrow();
  });

  it('accepts boundary latitude 90', () => {
    expect(() =>
      Location.create({ address: 'North Pole', latitude: 90, longitude: 0 }),
    ).not.toThrow();
  });

  it('accepts boundary longitude -180', () => {
    expect(() =>
      Location.create({ address: 'Date Line', latitude: 0, longitude: -180 }),
    ).not.toThrow();
  });
  ...
  it('throws InvalidLocationError for latitude > 90', () => {
    expect(() =>
      Location.create({ address: 'Nowhere', latitude: 91, longitude: 0 }),
  ...
  it('throws InvalidLocationError for longitude > 180', () => {
    expect(() =>
      Location.create({ address: 'Nowhere', latitude: 0, longitude: 181 }),
    ).toThrow(InvalidLocationError);
  });
  ...
  it('throws InvalidLocationError for empty address', () => {
    expect(() =>
      Location.create({ address: '', latitude: 0, longitude: 0 }),
    ).toThrow(InvalidLocationError);
  });
